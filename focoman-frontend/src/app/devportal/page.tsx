...
export default function DevPortalHomePage() {
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    e.preventDefault();
    const module = formModule === "CUSTOM" ? customModule : formModule;
    if (!module.trim()) {
      alert("Please enter a custom module name");
      return;
    }
    try {
      await devPortalApi.createTask(formTitle, formDescription, formType, formPriority, reportedBy, module);
      setFormTitle("");
      setFormDescription("");
      setFormType("TASK");
        ...
        <div className="w-1/2 flex flex-col h-full overflow-y-auto bg-white">
          ...
          <form onSubmit={handleCreateTask} className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {formModule === "CUSTOM" && (
                <div>
                  <label className="block text-xs font-bold text-text-secondary mb-1">Custom Module Name</label>
                  <input
                    type="text"
                    value={customModule}
