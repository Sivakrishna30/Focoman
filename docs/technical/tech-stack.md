...
| **Authentication** | **Firebase Authentication** | Auth for Studio Owner, Studio Members, and Customer Order Tracking (Guest Passkey). |
| **Primary Database** | **Google Cloud Firestore** | Document-oriented database with structured collections (`studios`, `members`, `customers`, `orders`, `tasks`). |
